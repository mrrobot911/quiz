...
export class QuestionComponent {
    ...
    isDisabled = computed(() => this.questionService.isActive() ? !this.questionService.state().canSubmit : this.questionService.state().isSubmiting, this);
    isTimeout = computed(() => !this.questionService.isActive(), this);
    buttonText = computed(() => !this.questionService.isActive() ? "Next question" : "Confirm answer", this);
    ...
    getAnswerClass(index: number): string {
        ...
        if (this.questionService.isActive()) {
            return this.selected() === index
                ? `${baseClass} questions__option--checked`
                : baseClass;
        }
        ...
        } else if (this.questionService.state().selected === index && index !== this.questionService.correctAnswer()) {
