...
export class QuizService {
    ...
    init() {
        return this.apiService.getSession()
            .then(resp => resp.has_active_game);
    }

    start() {
        return this.apiService.startQuiz()
            .then(resp => {
                this.updateGameState(resp);
                resp.next_question && this.updateQuestionState(resp.next_question);
    ...
    answer(index: number) {
        ...
        return this.apiService.postAnswer(currentIndex, currentNumber, questionId)
            .then(resp => {
